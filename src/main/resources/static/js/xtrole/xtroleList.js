    ...
    $("#grid-table").jqGrid({
		...
		},
		{
            ...
            formatter: function (value, grid, rows, state) {
                var html = '';
                if ($("#changjia_role_permission").val()) {//权限控制
            ...
            formatter: function (value, grid, rows, state) {
                var html = '';
                if ($("#changjia_role_user").val()) {//权限控制
...
function toUpdate(id) {
	top.dialog({
		url: basePath+'/xtrole/toUpdate?id='+id,
		title: '修改',
		width:400,
    	height:180,
		data: '', // 给modal 要传递的 的数据
...
function deleteRole(id) {
	$.post(basePath+"/xtrole/delete?id="+id, '', function(data) {
		  if (data.code == 0) {
...
function rolePermissionManage(id) {
	top.dialog({
		url: basePath+'/xtrole/rolePermissionManage?id='+id,
		title: '角色权限管理',
		width:800,
    	height:470,
		data: '', // 给modal 要传递的 的数据
...
function roleUserManage(id) {
	top.dialog({
		url: basePath+'/xtrole/roleUserManage?id='+id,
		title: '角色人员管理',
		width:700,
    	height:470,
		data: '', // 给modal 要传递的 的数据
