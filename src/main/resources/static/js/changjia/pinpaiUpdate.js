...
//查询生产厂家是否已存在
function pingpaiList(shengchanchangjia){
	 $.ajax({
		 url: basePath+'/newChangjia/findchangjia?shengchanchangjia='+ shengchanchangjia,
	        dataType: 'json',
	        contentType: "application/json",
	        jsonp: 'cb', //回调函数的参数名(键值)key
	        async: false,
...
function updatePinpaiChangJia(){
	...
	if (result){ 
	 $.ajax({
        url: basePath+'/newChangjia/updateChangJia?id='+id+"&shengchanchangjia="+shengchanchangjia+"&pinpaichangjia="+pinpaichangjia,
        type: 'POST', //GET
