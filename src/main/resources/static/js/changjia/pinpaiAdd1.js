$(function() {
	...
	$("#formSubmit").click(function(){
		...
		if (result){
			$.ajax({
	            url: basePath+'/newChangjia/changJiaAdd?shengchanchangjia='+shengchanchangjia+"&pinpaichangjia="+pinpaichangjia+"&companyid="+companyid,
	            type: 'POST', //GET
	            ...
	            success: function (data) {
	            	if(data==2){
	            		btn_alertDialog("提示","该生产厂家和品牌厂家已存在，请勿重复添加！");
	            	}else{
	            	   console.log(data);
	                   if (data) {
		                	if(data.code != 0){
...
$(function(){
    ...
    $('#shengchanchangjia').keyup(function(){
    	...
    	var companyid = $("#companyid").val();
        $.ajax({
        	url: basePath+'/newChangjia/findchangjia?pinpaichangjia='+ pinpaichangjia+"&companyid="+companyid,
            dataType: 'json',
            contentType: "application/json",
...
$(function(){
    ...
    $('#pinpaichangjia').keyup(function(){
        ...
        var companyid = $("#companyid").val();
        $.ajax({
        	url: basePath+'/newChangjia/findBrandBychangjia?pinpaichangjia='+ pinpaichangjia+"&companyid="+companyid,
            dataType: 'json',
            contentType: "application/json",
            jsonp: 'cb', //回调函数的参数名(键值)key
            // jsonpCallback: 'fun', //回调函数名(值) value
