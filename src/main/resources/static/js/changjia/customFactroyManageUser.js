...
$("#queryBtn").click(function(){
	$("#unCheckUL").html("");
	$.post("findallUser", $("#searchForm").serialize(), function(data) {
...
$("#subBtn").click(function(){
	...
	if (userIds.length==0) {
		confirmDialog("确认信息","确定清空该厂家下所有用户？",saveRoleUsers,userIds);
	}else{
...
function saveRoleUsers(userIds) {
	//提交修改
	$.post("addUserCodes", {userIds:userIds,factoryCode:$("#factoryCode").val()}, function(data) {
		  if (data.code == 0) {
