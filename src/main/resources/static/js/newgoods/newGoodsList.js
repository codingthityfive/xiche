...
$(document).ready(function () {
    ...
    $("#grid-table").jqGrid({
        ...
        ondblClickRow:function(rowId){
            ...
            top.dialog({
                url: basePath+'/newgoods/toFind?billId='+billId,
                title: '单据详情',
                width:900,
        ...
        onSelectRow:function(rowId){
            ...
            // global_billId = $("#grid-table").getCell(rowId, "billId");
            // global_status = $("#grid-table").getCell(rowId, "status");
    ...
    $("#resetBtn").click(function(){
        ...
        $("#goodsName").val("");
        $("#status").val("");
        window.location.href=basePath+"/newgoods/toList";
    });

    /*//查看按钮
    $("#newgood_see_btn").click(function () {
        if(global_billId == undefined){
            btn_alertDialog("提示","请选择一条记录！");
            return;
        }
        toFind(global_billId);
    });
    //审核按钮
    $("#newgood_audit_btn").click(function () {
        if(global_billId == undefined){
            btn_alertDialog("提示","请选择一条记录！");
            return;
        }
        if(status != 3){
            btn_alertDialog("提示","该单据已审核！");
            return;
        }
        toAudit(global_billId);
    });*/
...
function toAudit(billId) {
    top.dialog({
        url: basePath+'/newgoods/toAudit?billId='+billId,
        title: '单据详情',
        width:900,
        height:600,
