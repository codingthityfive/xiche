...
$(document).ready(function () {
    ...
    $("#grid-table").jqGrid({
        ...
        //mtype:"GET",//提交方式
        colNames: [ '活动商品','商品名称','规格','生产厂家','库存','单品优惠金额（元）'],
        colModel: [{
            name: 'goodsCode',
            index: 'goodsCode',
        },{
        ...
        autowidth: true,//自动宽
        loadComplete: function() {
            var table = $(this).jqGrid("getRowData");
            var sumdiscount=0;
            for (i = 0; i < table.length; i++)
            {
                sumdiscount = parseFloat(table[i].discount)+sumdiscount;
            }
            document.getElementById('yhje').innerHTML=sumdiscount;
            console.info("adsfas"+sumdiscount)
        },
